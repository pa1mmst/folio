...
async def view_note(name: str, request: Request):
    note = read_note(name)
    if not note:
        return render_page(request, "Not found", '<div class="empty"><p>Note not found.</p><a href="/" class="back-link">Back to notes</a></div>')
...
async def edit_note(name: str, folder: str = "", request: Request = None):
    ...
    """
    return render_page(request, f"Edit: {name}", body)
...
async def graph_page(request: Request):
    ...
    """
    return render_page(request, "Graph", body, active="graph")
